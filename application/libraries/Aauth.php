...
class Aauth {
	...
	private function precache_perms() {
		$query	= $this->default_db->get($this->config_vars['perms']);
	...
	private function precache_groups() {
		$query	= $this->default_db->get($this->config_vars['groups']);
	...
	public function login($identifier, $pass, $remember = FALSE, $totp_code = NULL) {
		...
		if ( $query->num_rows() != 0 && $this->verify_password($password, $row->pass) ) {
			...
			$this->add_member($row->id, $this->config_vars['default_group']);
	...
	public function get_user_groups($user_id = FALSE){
		...
		if( !$user_id){
			$this->default_db->where('name', $this->config_vars['public_group']);
			$query = $this->default_db->get($this->config_vars['groups']);
		}else if($user_id){
			$this->default_db->join($this->config_vars['groups'], "id = group_id");
			$this->default_db->where('user_id', $user_id);
			$query = $this->default_db->get($this->config_vars['user_to_group']);
		}
		return $query->result();
	}
	...
	public function get_user_perms ( $user_id = FALSE ) {
		...
		if($user_id){
			$query = $this->default_db->select($this->config_vars['perms'].'.*');
			$query = $this->default_db->where('user_id', $user_id);
			$query = $this->default_db->join($this->config_vars['perms'], $this->config_vars['perms'].'.id = '.$this->config_vars['perm_to_user'].'.perm_id');
			$query = $this->default_db->get($this->config_vars['perm_to_user']);
	...
	public function create_group($group_name, $definition = '') {
		...
		$query = $this->default_db->get_where($this->config_vars['groups'], array('name' => $group_name));
		...
		if ($query->num_rows() < 1) {
			...
			$this->default_db->insert($this->config_vars['groups'], $data);
			$this->precache_groups();
	...
	public function update_group($group_par, $group_name=FALSE, $definition=FALSE) {
		...
		return $this->default_db->update($this->config_vars['groups'], $data);
	}
	...
	public function delete_group($group_par) {
		...
		$this->default_db->where('id',$group_id);
		$query = $this->default_db->get($this->config_vars['groups']);
		if ($query->num_rows() == 0){
			return FALSE;
		}

		$this->default_db->trans_begin();

		// bug fixed
		// now users are deleted from user_to_group table
		$this->default_db->where('group_id', $group_id);
		$this->default_db->delete($this->config_vars['user_to_group']);

		$this->default_db->where('group_id', $group_id);
		$this->default_db->delete($this->config_vars['perm_to_group']);

		$this->default_db->where('group_id', $group_id);
		$this->default_db->delete($this->config_vars['group_to_group']);

		$this->default_db->where('subgroup_id', $group_id);
		$this->default_db->delete($this->config_vars['group_to_group']);

		$this->default_db->where('id', $group_id);
		$this->default_db->delete($this->config_vars['groups']);

		if ($this->default_db->trans_status() === false) {
			$this->default_db->trans_rollback();
			return false;
		} else {
			$this->default_db->trans_commit();
			$this->precache_groups();
			return true;
		}
	...
	public function add_member($user_id, $group_par) {
		...
		$query = $this->default_db->where('user_id',$user_id);
		$query = $this->default_db->where('group_id',$group_id);
		$query = $this->default_db->get($this->config_vars['user_to_group']);
		...
		if ($query->num_rows() < 1) {
			...
			return $this->default_db->insert($this->config_vars['user_to_group'], $data);
		}
	...
	public function remove_member($user_id, $group_par) {
		...
		$group_par = $this->get_group_id($group_par);
		$this->default_db->where('user_id', $user_id);
		$this->default_db->where('group_id', $group_par);
		return $this->default_db->delete($this->config_vars['user_to_group']);
	}
	...
	public function add_subgroup($group_par, $subgroup_par) {
		...
		$query = $this->default_db->where('group_id',$group_id);
		$query = $this->default_db->where('subgroup_id',$subgroup_id);
		$query = $this->default_db->get($this->config_vars['group_to_group']);
		...
		if ($query->num_rows() < 1) {
			...
			return $this->default_db->insert($this->config_vars['group_to_group'], $data);
		}
	...
	public function remove_subgroup($group_par, $subgroup_par) {
		...
		$this->default_db->where('group_id', $group_par);
		$this->default_db->where('subgroup_id', $subgroup_par);
		return $this->default_db->delete($this->config_vars['group_to_group']);
	}
	...
	public function get_group_name($group_id) {
		...
		$query = $this->default_db->where('id', $group_id);
		$query = $this->default_db->get($this->config_vars['groups']);
	...
	public function get_group_perms ( $group_par ) {
		if ($group_id = $this->get_group_id($group_par)) {
			$query = $this->default_db->select($this->config_vars['perms'].'.*');
			$query = $this->default_db->where('group_id', $group_id);
			$query = $this->default_db->join($this->config_vars['perms'], $this->config_vars['perms'].'.id = '.$this->config_vars['perm_to_group'].'.perm_id');
			$query = $this->default_db->get($this->config_vars['perm_to_group']);
	...
	public function get_subgroups ( $group_par ) {
		...
		$query = $this->default_db->where('group_id', $group_id);
		$query = $this->default_db->select('subgroup_id');
		$query = $this->default_db->get($this->config_vars['group_to_group']);
	...
	public function create_perm($perm_name, $definition='') {
		...
		$query = $this->default_db->get_where($this->config_vars['perms'], array('name' => $perm_name));
		...
		if ($query->num_rows() < 1) {
			...
			$this->default_db->insert($this->config_vars['perms'], $data);
			$this->precache_perms();
			return $this->default_db->insert_id();
		}
	...
	public function update_perm($perm_par, $perm_name=FALSE, $definition=FALSE) {
		...
		$this->default_db->where('id', $perm_id);
		return $this->default_db->update($this->config_vars['perms'], $data);
	}
	...
	public function delete_perm($perm_par) {
		...
		$this->default_db->where('perm_id', $perm_id);
		$this->default_db->delete($this->config_vars['perm_to_group']);

		// deletes from perm_to_user table
		$this->default_db->where('perm_id', $perm_id);
		$this->default_db->delete($this->config_vars['perm_to_user']);

		// deletes from permission table
		$this->default_db->where('id', $perm_id);
		$this->default_db->delete($this->config_vars['perms']);
	...
	public function list_group_perms($group_par) {
		...
		$this->default_db->from($this->config_vars['perms']);
		$this->default_db->join($this->config_vars['perm_to_group'], "perm_id = ".$this->config_vars['perms'].".id");
		$this->default_db->where($this->config_vars['perm_to_group'].'.group_id', $group_par);
	...
	public function is_allowed($perm_par, $user_id=FALSE){
		...
		$query = $this->default_db->where('perm_id', $perm_id);
		$query = $this->default_db->where('user_id', $user_id);
		$query = $this->default_db->get( $this->config_vars['perm_to_user'] );
	...
	public function is_group_allowed($perm_par, $group_par=FALSE){
		...
		if($group_par != FALSE){
			...
			$group_par = $this->get_group_id($group_par);
			$query = $this->default_db->where('perm_id', $perm_id);
			$query = $this->default_db->where('group_id', $group_par);
			$query = $this->default_db->get( $this->config_vars['perm_to_group'] );
	...
	public function allow_user($user_id, $perm_par) {
		...
		if ($query->num_rows() < 1) {
			...
			return $this->default_db->insert($this->config_vars['perm_to_user'], $data);
		}
		return TRUE;
	}
	...
	public function deny_user($user_id, $perm_par) {
		...
		return $this->default_db->delete($this->config_vars['perm_to_user']);
	}
	...
	public function allow_group($group_par, $perm_par) {
		...
		if ($query->num_rows() < 1) {
			...
			return $this->default_db->insert($this->config_vars['perm_to_group'], $data);
		}
	...
	public function deny_group($group_par, $perm_par) {
		...
		return $this->default_db->delete($this->config_vars['perm_to_group']);
	}
	...
	public function get_perm($perm_par) {
		if ($perm_id = $this->get_perm_id($perm_par)) {
			$query = $this->default_db->where('id', $perm_id);
			$query = $this->default_db->get($this->config_vars['perms']);
